...
def guess_table(page):
    ...
    for idx in range(count):
        ...
        try:
            ...
            header_text = ""
        if "Company" in header_text or "Trade" in header_text:
            return table
    if count > 0:
        return tables.first
    return None
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for keyword in keywords:
            ...
            input_box = first_visible(page, DUBAI_DED_SELECTORS["search_inputs"])
            if not input_box:
                logging.warning("Search input not found for keyword %s", keyword)
                continue
            input_box.fill(keyword)

            search_button = first_visible(page, DUBAI_DED_SELECTORS["search_buttons"])
            if search_button:
                search_button.click()
            else:
            ...
            while True:
                ...
                for values, header_map in extract_table_rows(table):
                    company = find_row_value(values, header_map, ["Company", "Trade"])
                    activity = find_row_value(values, header_map, ["Activity", "Business", "License"])
                    record = {
                        ...
                        "notes": "ded_public_lookup_no_contact",
                    }
